...
public:
    ...
    //[EDITING FUNCTIONS]--------------------------------------------------
    // sets cursorLine 
    void setCursorLine(const unsigned int& newLin);

    //sets cursorColumn
    void setCursorColumn(const unsigned int& newCol);

    //inserts string into line
    void insert(const std::string& somestring);

    //delete from starting index  to end index 
    void deleting(unsigned int start, unsigned int numChars);

    //creates newline 
    void createNewLine(const std::string& beyondCursor);
